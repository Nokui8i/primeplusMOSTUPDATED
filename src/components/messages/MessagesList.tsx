...
export function MessagesList({ threadId, filter, sortBy }: MessagesListProps) {
  ...
  // Fetch user profiles for all unique senderIds in messages
  useEffect(() => {
    async function fetchProfiles() {
      const uniqueSenderIds = Array.from(new Set(messages.map(m => m.senderId)));
      const missingIds = uniqueSenderIds.filter(id => !userProfiles[id]);
      if (missingIds.length === 0) return;
      const profiles: Record<string, { displayName: string; username: string; photoURL?: string }> = {};
      for (const id of missingIds) {
        const userSnap = await getDocs(query(collection(db, 'users'), where('uid', '==', id)));
        if (!userSnap.empty) {
          const userData = userSnap.docs[0].data();
          profiles[id] = {
            displayName: userData.displayName || userData.username || id,
            username: userData.username || id,
            photoURL: userData.photoURL || undefined,
          };
        } else {
          profiles[id] = { displayName: id, username: id };
        }
      }
  ...
  return (
    <div className="flex-1 overflow-y-auto p-4 space-y-4">
      ...
      {filteredMessages.map((message) => (
        ...
        >
          {/* User Avatar */}
          <div className="w-10 h-10 rounded-full bg-gray-100 relative overflow-hidden flex items-center justify-center">
            {message.senderId === user?.uid ? (
              ...
              <Image
                ...
                alt={userProfiles[message.senderId].displayName}
                fill
                className="object-cover"
              />
            ) : (
              <span className="text-lg font-bold text-gray-500">
                {userProfiles[message.senderId]?.displayName?.charAt(0).toUpperCase() || message.senderId.charAt(0).toUpperCase()}
              </span>
            )}
          </div>
          ...
          <div className={`flex-1 space-y-1 ${
            ...
            {/* Message Actions */}
            {message.senderId === user?.uid && !(message as any).isDeleted && !editingMessageId && (
