...
export class MessageService {
      ...
      if (attachments && attachments.length > 0) {
        ...
        messageAttachments = await Promise.all(
          attachments.map(async (file) => {
            const s3Key = generateS3Key(threadId, file.name, 'documents');
  ...
  subscribeToUserPresence(userId: string, callback: (presence: UserPresence) => void) {
    ...
    return onSnapshot(presenceRef, (doc) => {
      ...
      // For real-time updates, trust the status from Firebase
      // The AuthContext already handles setting offline when user disconnects
  ...
  private async uploadAudio(blob: Blob): Promise<string> {
    // Implement audio upload to AWS S3
    const { uploadToS3, generateS3Key } = await import('@/lib/aws/s3');
    const s3Key = generateS3Key('audio', `${uuidv4()}.mp3`, 'audio');
    return await uploadToS3(blob as File, s3Key);
  }
