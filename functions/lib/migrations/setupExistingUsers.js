...
exports.setupExistingUsers = void 0;
...
exports.setupExistingUsers = functions.https.onRequest(async (request, response) => {
    ...
    try {
        ...
        for (const userDoc of usersSnapshot.docs) {
            ...
            if (!userData.email)
                userUpdateData.email = null; // or some default
            if (!userData.displayName)
                userUpdateData.displayName = userData.email?.split('@')[0] || 'Existing User';
            if (!userData.photoURL)
                userUpdateData.photoURL = null;
            if (!userData.createdAt)
                userUpdateData.createdAt = admin.firestore.FieldValue.serverTimestamp(); // If somehow missing
            if (typeof userData.followersCount !== 'number' || userData.followersCount < 0)
                userUpdateData.followersCount = 0;
            if (typeof userData.followingCount !== 'number' || userData.followingCount < 0)
                userUpdateData.followingCount = 0;
            ...
            try {
                ...
                errors.push(`User ${userId}: ${batchError.message}`);
            }
        }
...
//# sourceMappingURL=setupExistingUsers.js.map